              ...
              {leaderboard.map((entry, index) => (
                <div key={index} className="py-4 px-6 flex justify-between items-center hover:bg-yellow-900/20 transition-colors">
                  <span className="flex items-center">
                    <span className={`mr-4 w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${
                      index === 0 ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 text-black shadow-[0_0_15px_rgba(255,215,0,0.8)]' :
                      index === 1 ? 'bg-gradient-to-r from-gray-300 to-gray-500 text-black shadow-[0_0_10px_rgba(192,192,192,0.6)]' :
                      index === 2 ? 'bg-gradient-to-r from-amber-500 to-amber-700 text-white shadow-[0_0_10px_rgba(217,119,6,0.6)]' :
                      'bg-[#2a1600] text-yellow-200 border-2 border-yellow-800'
                    }`}>
